...
**Windows users**: Launch from RStudio or PowerShell. Git Bash is not supported due to
known R/MSYS2 compatibility issues. From PowerShell:
```powershell
Rscript -e "library(MicrobiomeExplorer); runMicrobiomeExplorerApp()"
```
...
```
The Vignettes will provide in depth documentation on how you can use the shinyapp
to validate, normalize, analyze, and visualize the data. You can get the data in the
`data/` directory as both will work, download from my respective GitHub section as
well.

The package also contains one sample microbiome dataset presented as a dataframe
...
- **phyloseq**: Enhances data management, particularly in functions like `calculateStats` and `calculateAlphaDiversity`, offering robust handling of complex microbiome datasets.

- **ggplot2**: Underpins the visualization aspects in `plotMicrobiomeHeatmap` and `advancedRarefactionPlot`, providing elegant and informative data representation.

- **vegan**: Contributes to diversity analysis, especially in `calculateAlphaDiversity` and `advancedRarefactionPlot`, delivering extensive ecological insights.
