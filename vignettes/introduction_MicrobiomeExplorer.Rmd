---
title: "A Detailed Guide On Using the Functions in the MicrobiomeExplorer Package"
...
**This document provides a guide to using the MicrobiomeExplorer package (version 0.2.0)**.
...
In this example, we will use the `calculateStats` function on a sample dataset to demonstrate its functionality.
...
Now, we will use the `calculateStats` function to calculate extended statistics for the Iris dataset:

```{r calculate-stats}
# Calculate statistics
stats <- calculateStats(iris_data)
stats
```
...
The `calculateStats` function from the MicrobiomeExplorer package is an essential tool for conducting preliminary statistical analysis on microbiome datasets. It provides valuable insights into the distribution and variability of features within your data.
...
The `advancedRarefactionPlot` function generates true rarefaction curves
by estimating expected species richness at a range of sequencing depths
for each sample. This is the standard quality-control visualization for
assessing whether samples have been sequenced deeply enough to capture
the community's diversity.

### Function Parameters

- `data`: A matrix or dataframe of integer counts (rows = samples, columns = taxa).
- `step`: Optional integer spacing between sequencing depths.
- `n_steps`: Number of evenly-spaced depths when `step` is not supplied (default 50).

### Plotting with `advancedRarefactionPlot`

We use the `microbiomeExample` dataset (integer counts) to build rarefaction curves.
Each line shows one sample; the vertical dashed line marks the minimum library size
across all samples (a common rarefaction depth).

```{r plot-rarefaction}
# microbiomeExample contains whole-number counts required by vegan::rarefy
advancedRarefactionPlot(microbiomeExample)
```

Samples whose curves plateau early have been sequenced to saturation.
Samples still climbing may benefit from deeper sequencing.





### Introduction to `plotMicrobiomeHeatmap` function

The `plotMicrobiomeHeatmap` function is designed for advanced visualization of microbiome data. It offers features such as data normalization, various clustering methods, and enhanced visualization options using heatmaps. This function is crucial for exploring complex microbiome datasets and uncovering underlying patterns.
...
Let's create a heatmap using the `plotMicrobiomeHeatmap` function with a mock microbiome dataset.

### Creating the Heatmap

Now, we will use the `plotMicrobiomeHeatmap` function to visualize this data:

```{r create-heatmap}
# Plot the heatmap with dynamic color palette
heatmap_plot <- plotMicrobiomeHeatmap(microbiome_example, normalize = TRUE)
print(heatmap_plot)
...
The `plotMicrobiomeHeatmap` function is an invaluable tool for microbiome researchers and data scientists. It provides an effective way to visualize complex datasets, helping to identify patterns, correlations, and other significant insights in microbiome data.


### Introduction to calculateAlphaDiversity Function

The `calculateAlphaDiversity` function is a crucial tool in microbiome data analysis, designed to compute various alpha diversity indices. These indices are key in understanding the complexity and richness of microbial communities within samples. The function's ability to handle both standard and rarefied data makes it versatile and suitable for a broad range of datasets.
...
In addition to the Differential Expression Analysis, the MicrobiomeExplorer Shiny app features an Advanced Alpha Diversity Analysis module, designed to calculate various alpha diversity indices for microbiome data. This module leverages the `calculateAlphaDiversity` function, enabling a comprehensive assessment of species diversity within each sample.

#### Calculate Alpha Diversity
- **Function**: `calculateAlphaDiversity`
...
The Heatmap Visualization module in the MicrobiomeExplorer Shiny app is an essential tool for graphically representing microbiome data. Utilizing the `plotMicrobiomeHeatmap` function, this module creates comprehensive heatmaps that offer insights into the abundance and distribution of taxa across samples. Here's a detailed look at the function and its application in the app:

#### Plot Microbiome Heatmap
- **Function**: `plotMicrobiomeHeatmap`
